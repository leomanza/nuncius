...
**Live demo proposal (#6):** *"Should the Nuncius DAO commit 50,000 USDC to a quarterly grant cycle for independent ZK research, prioritizing Semaphore V5 contributions?"* → **APPROBATVR 5–0** at block 30807142 — see the DisputeDAO event log on [chainscan-galileo](https://chainscan-galileo.0g.ai/address/0x650c074910bC5855f6573f9d62EE5b8bA90664D9).
...
3. **Adversarial testing on-chain.** Cross-proposal replay, bogus signal, and nullifier reuse all revert on Galileo (see `logs/session4-attacks.log`).
...
DAO group id: **1** · 5 voter commitments registered · 6 proposals opened on-chain to date (3 demo, 1 attack-test, 2 cleanup).
...
Each subname carries 10 text records, including `semaphore.pubkey` (Baby Jubjub, 128 hex chars) and `semaphore.commitment` (decimal). 50/50 verified by direct `PublicResolver.text()` reads — see `logs/ens-nuncius-verify.log`.
...
- **Multi-hop routing verified**: node-2 → node-5 via node-1, captured in `logs/axl-mesh-boot.log`.
- **No central broker.** AXL has no GossipSub primitive; the coordinator script fans out by iterating peers, all over the encrypted mesh. Documented as a doc gap in `FEEDBACK.md`.
- **Build provenance:** github.com/gensyn-ai/axl @ commit 9cba555, built locally with go1.26.2.

### 0G — *swarm settlement layer*
- **0G Chain (Galileo testnet)** hosts the entire DAO + Semaphore stack. BN254 pairing precompiles work (real Groth16 fixture verified on-chain in Session 1).
- **0G Storage KV** holds each agent's working memory (`key = keccak256(daoAddress ‖ "agent" ‖ i)` so streams never collide across deployments). 10+ KV writes captured in Session 3 logs.
- **0G Compute** drives deliberation. Discovered the catalogued model `deepseek-ai/DeepSeek-R1-70B` is offline on Galileo — agents pick the live `qwen/qwen-2.5-7b-instruct` provider via `listService()` (documented for future builders in `FEEDBACK.md`).
...
├── FEEDBACK.md               — sponsor DX feedback
├── DEPLOY_LOG.md             — every deploy + tx, session-by-session
├── MASTER_PLAN.md            — original solo build plan
...
│   └── scripts/              — trigger-proposal, setup-agent-wallets
...
└── logs/                     — every test-run output, screenshots, peer-id snapshots
```

---

## Test runs (all on Galileo, all in `DEPLOY_LOG.md`)

| Run | Proposal | Outcome | Notes |
|---|---|---|---|
| Run 1 | #1 — 50k audit smoke | Approved 3-2 | Single signer, all 5 votes (Session 2 bonus E2E) |
| Run 4 | #3 — same audit, real agents | All 5 deliberate, all Approve | 5 distinct agent wallets |
| Run 5 | #3 — anonymous votes on-chain | Approved 5-0 | Each agent signs from its own wallet |
| Run 6 | #4 — adversarial | All 3 attacks revert | B5 wrong-scope, B4 invalid signal, nullifier reuse |
| Demo  | #6 — ZK research grant | **APPROBATVR 5-0** | Captured in screenshots above |
