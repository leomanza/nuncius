...
import { DeliberationResult } from "../../shared/types";
...
export async function deliberate(
...
): Promise<DeliberationResult & { source: string; elapsedMs: number; rawText: string; displayName: string }> {
  const p = getPersonaByIndex(agentIndex);
  ...
  const result = await callComputeInference(system, user, { maxTokens: 400, temperature: 0 });
